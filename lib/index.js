...
NwBuilder.prototype.handleMacApp = function () {
    ...
    this._forEachPlatform(function (name, platform) {
        ...
        var icons = self.options.macIcns;

        // Let's first handle the mac icon
        if(icons) {
            if (typeof icons === 'string') {
              allDone.push(Utils.copyFile(icons, path.resolve(self.getResourcesDirectoryPath(platform), 'app.icns'), self));
            }

            if (typeof icons === 'object' && !Array.isArray(icons) && icons !== null) {
              Object.keys(icons).forEach(function (key) {
                var iconName = key + '.icns';

                allDone.push(Utils.copyFile(icons[key], path.resolve(self.getResourcesDirectoryPath(platform), iconName), self));
              });
            }
        }

        allDone.push(rm(path.resolve(self.getResourcesDirectoryPath(platform), '*.lproj')));
